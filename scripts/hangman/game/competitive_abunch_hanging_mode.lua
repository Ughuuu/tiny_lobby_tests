...
function CompetitiveAbunchHanging:check_game_end(l)
    local all_lost = true
    for k, _ in pairs(l.peers) do
    ...
    if all_lost then
        ...
        if #winners > 1 then
            lobby.broadcast_chat(string.format("The winners are: %s", table.concat(winners, ", ")))
        else
            lobby.broadcast_chat(string.format("%s is the winner!", winners[1]))
        end
        l.public_data["game_state"] = "over"
...
function CompetitiveAbunchHanging:on_timer_next_round(l)
    local l_new, word, hint = self:get_competitive_word(l)
    l = l_new
    if l.error then return l end
    l.private_data["word"] = word
...
function CompetitiveAbunchHanging:on_timer_restart_game(l)
    for k, _ in pairs(l.peers) do
...
function CompetitiveAbunchHanging:on_timer_guess_timeout(l)
    -- Placeholder for guess timeout logic
end

function CompetitiveAbunchHanging:set_initial_data(l)
...
function CompetitiveAbunchHanging:get_competitive_word(l)
    while true do
        local retry = false
        local data = {
            lobby_type = "10v10",
...
function CompetitiveAbunchHanging:check_single_winner(l)
